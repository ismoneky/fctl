    ...
    <view class="profile">
                ...
                <view class="apply-modal-body">
                    <view class="apply-form-item">
                        <view class="apply-input-wrap">
                            ...
                            <input class="apply-input" v-model="applyForm.name" placeholder="请输入您的姓名" placeholder-style="color:#bbb" maxlength="20" />
                        </view>
                    </view>
                    <view class="apply-form-item">
                        <view class="apply-input-wrap">
                            <text class="apply-input-icon">📱</text>
                            <input class="apply-input" v-model="applyForm.phone" placeholder="请输入手机号码" placeholder-style="color:#bbb" type="number" maxlength="11" />
                        </view>
                    </view>
                </view>
                <view class="apply-modal-btns">
                    <view class="apply-btn apply-btn-cancel" @tap="closeApplyModal">取消</view>
                    <view class="apply-btn apply-btn-submit" @tap="submitApply">提交申请</view>
                </view>
            </view>
        </view>
        ...
        <view v-if="verifyModal.show" class="modal-mask" @tap="closeVerifyModal">
            <view class="modal-card verify-modal" @tap.stop>
                <view class="verify-result-icon" :class="verifyModal.success ? 'icon-success' : 'icon-fail'">
                    <text class="verify-result-emoji">{{ verifyModal.success ? '✓' : '✕' }}</text>
                </view>
                <text class="verify-modal-title" :class="verifyModal.success ? 'title-success' : 'title-fail'">
                    {{ verifyModal.success ? '核验成功' : '核验失败' }}
                </text>
                <text class="verify-modal-msg">{{ verifyModal.message }}</text>
                <view class="verify-modal-btn" :class="verifyModal.success ? 'btn-success' : 'btn-fail'" @tap="closeVerifyModal">确定</view>
            </view>
        </view>
        ...
        <view class="container">
            ...
            <view class="user-card">
                <view class="user-header">
                    <image class="avatar" :src="userInfo.avatar" mode="aspectFill" @tap="onAvatarTap"></image>
    ...
    methods: {
        ...
        submitApply() {
            ...
            request({
                ...
                this.applyModal.show = false;
                uni.showToast({ title: '申请已提交，请等待审核', icon: 'none', duration: 2500 });
            }).catch(() => {
...
.title-success { color: #2dc76d; }
.title-fail    { color: #f5515f; }
